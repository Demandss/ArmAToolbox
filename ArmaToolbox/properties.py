...
class ArmaToolboxProperties(bpy.types.PropertyGroup):
    ...
    animSource: bpy.props.StringProperty(name="Animation source", default="reloadMagazine",
                                         description="Name of the animation source")
    prefixString: bpy.props.StringProperty(name="Output Prefix String", default="",
                                           description="String to prefix output with")
...
class ArmaToolboxGUIProps(bpy.types.PropertyGroup):
    ...
    # Mlod folder
    mlodDayZFolder: bpy.props.StringProperty("mlodDayZFolder", description="Path to DZ folder", subtype='DIR_PATH')
    mlodSuffix: bpy.props.StringProperty("mlodSuffix", description="Suffix mlod, if not, empty string", default="_mlod")
    mlodEmptyProxy: bpy.props.BoolProperty(name="Use empty proxy", description="Use empty proxy", default=False)
    mlodEmptyProxyFile: bpy.props.StringProperty("mlodEmptyProxyFile", description="Mlod for empty proxy",
                                                 subtype='FILE_PATH')

    # Material Relocator
    matOutputFolder: bpy.props.StringProperty("matOutputFolder", description="RVMat output", subtype='DIR_PATH')
    matAutoHandleRV: bpy.props.BoolProperty(name="matAutoHandleRV", description="Automatically relocate RVMat",
                                            default=True)
    matPrefixFolder: bpy.props.StringProperty("matPrefixFolder", description="Prefix for texture search",
                                              subtype='DIR_PATH',
                                              default="P:\\")
    # Merge as Proxy
    ...
    # Selection Tools
    hiddenSelectionName: bpy.props.StringProperty("hiddenSelectionName",
                                                  description="Name of the hidden selection to create", default="camo")

    # Hitpoint creator
    hpCreatorSelectionName: bpy.props.StringProperty("hpCreatorSelectionName",
                                                     description="Name of the hitpoint selection create",
                                                     default="_point")
    hpCreatorRadius: bpy.props.FloatProperty("hpCreatorRadius", description="radius of hitpont sphere", default=0.3,
                                             min=0.001)
